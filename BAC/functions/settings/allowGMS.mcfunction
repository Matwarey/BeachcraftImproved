...
execute @s[tag=!op] ~~~ tellraw @a[tag=op] {"rawtext":[{"text":"§r§6[§aScythe§6]§r "},{"selector":"@s"},{"text":" has tried to toggle Gamemode 0 without perms!"}]}

# allow
execute @s[type=player,tag=op,scores={gms=..0}] ~~~ scoreboard players set scythe:config gms 1
execute @s[type=player,tag=op,scores={gms=..0}] ~~~ tellraw @a[tag=op] {"rawtext":[{"text":"§r§6[§aScythe§6]§r "},{"selector":"@s"},{"text":" has disallowed §4gamemode 0§r to be used!"}]}
